...
var toDo=[];
glob(`output/**/${process.argv[2]}?(Api|List|Repository|Service).java`).then(files =>{

    
    for(var id in files){
        var destArr=files[id].split('/');
        destArr.splice(0,3)
        if(files[id].startsWith( 'output/java/sisjuri/br/gov/pa/igeprev/sisjuri/core')){
            destArr.unshift('sisjuri-core/src/main/java/')
        }
        else{
            destArr.unshift('sisjuri-api/src/main/java/')
        }
        destArr.unshift('..')
